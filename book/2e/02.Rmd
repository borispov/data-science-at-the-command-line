...
The following command is the same as above, but then written on one line:
...
:   Shell builtins are command-line tools provided by the shell, which is the Z shell (or `zsh`) in our case. Examples include `cd` and `help`. These cannot be changed. Shell builtins may differ between shells. Like binary executables, they cannot be easily inspected or changed.
...
```{console, include=FALSE}
 alias bat='bat --tabs 8 --paging never --terminal-width 80 --theme "Solarized (dark)"'
```
...
We can *pipe* the output of `seq` to `grep` to filter lines on a pattern.
...
Here, we save the output of the `seq` tool to a file named *ten-numbers.txt* in the directory */data/ch02*.
...
```{console}
wc -w greeting.txt ten-numbers.txt
```

Note that in this case, `wc` also outputs the name of the files.


```{r piping, echo=FALSE, fig.cap="Piping output to another tool (placeholder)", fig.align="center"}
knitr::include_graphics("images/diagram_piping.png")
```

```{r redirection, echo=FALSE, fig.cap="Redirecting output to a file (placeholder)", fig.align="center"}
knitr::include_graphics("images/diagram_redirection.png")
```
...
Sometimes a tool or a sequence of tools produce too much output to include in the book.
...
To use `bat` to show the contents of a file where line numbers and syntax highlighting matters.
...
In Chapter 7, I explain four options for you to display generated images from the command line.
...
In that case, your best bet is to invoke the tool with the `--help` option or the short-hand version which is `-h`.
For example:
